...
int getRandom() {
	int number = Gamedata::getInstance().getRandInRange(0, (Gamedata::getInstance().getXmlInt("world/height") ));
	number *= rand() % 2 ? -1 : 1;
	return number;
}
...
Sprite::Sprite(const std::string& name, const Frame* const frame) :
  Drawable(name,
           ...
           Vector2f(Gamedata::getInstance().getXmlInt(name+"/speedX")+getRandom(), 
