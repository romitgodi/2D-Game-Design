...
void Health::drawReverse() const {
 drawBox();
  Draw_AALine(screen, start[0] + currentLength, start[1], 
                      start[0] + totalLength, start[1], 
                      thick, color);
}
...
void Health::decrementEnemy() {
	if (currentLength < totalLength) currentLength += (10 * increments);
}
