...
ScaledSprite::ScaledSprite(const std::string& name, SDL_Surface* surface, const int x) :
	Drawable(name,
		 Vector2f( (Gamedata::getInstance().getXmlInt(name+"/startLoc/x") + (x * ( static_cast<float>(Gamedata::getInstance().getXmlInt("world/width")) / (Gamedata::getInstance().getXmlInt("fire/count")) ) ) ), 
